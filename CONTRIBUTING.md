...
- The local Windows packaging scripts also stop a running packaged `Openscreen.exe` instance and clean `release/<version>` first, which avoids the common Windows file-lock failure when rebuilding.
