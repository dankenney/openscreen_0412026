...
- The local Windows packaging scripts also stop a running packaged `OpenScreen.exe` instance (including the older `Openscreen.exe` name) and clean `release/<version>` first, which avoids the common Windows file-lock failure when rebuilding.
